    ...
    try:
        ...
        if message_result.get("duplicate"):
            update_webhook_log(
                ...
                error_message="Duplicate zalo_msg_id"
            )
            return {
                "ok": True,
                "duplicate": True,
...
def update_conversation_summary(conversation_name, content, sent_at, sender_type):
    ...
    new_topic = guess_topic(content)

    # Nếu ban đầu chỉ là topic chung chung, cho phép cập nhật sang topic cụ thể hơn
    if (
        not conversation.topic
        or conversation.topic == "Tư vấn sản phẩm"
        or new_topic != "Tư vấn sản phẩm"
    ):
